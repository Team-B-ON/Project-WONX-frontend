...
const dummyMovies: Movie[] = Array.from({ length: 18 }, (_, idx) => ({
  ...
  genres: (['SF', '액션', '코미디'].slice(0, (idx % 3) + 1) as unknown) as Genre[],
}))
...
const MyPage: React.FC = () => {
  ...
  useEffect(() => {
    axiosInstance
      .get<PublicProfileDto>('/mypage')
      .then(res => setProfile(res.data))
      .catch(() => {
        // 로그인 안 돼 있으면 /login 이동 등 처리
      })
  }, [])
  ...
  const handleEditProfile = () => {
    ...
    setPreviewAvatar(profile.profileImageUrl)
  ...
  const saveNewName = () => {
    ...
    axiosInstance
      .patch<PublicProfileDto>('/mypage', { nickname: tempName })
      .then(res => {
        setProfile(res.data)
